...
# Import from MoGe
from nerfstudio.MoGe.moge.model import MoGeModel 
...
class DepthDataset(InputDataset):
    ...
    def __init__(self, dataparser_outputs: DataparserOutputs, scale_factor: float = 1.0):
        ...
        if len(dataparser_outputs.image_filenames) > 0 and (
            ...
            self.MoGe = MoGeModel.from_pretrained("Ruicheng/moge-vitl").to(device)
            ...
            for i in track(range(len(filenames)), description="Generating depth images"):
                ...
                with torch.no_grad():
                    ...
                    # print("Zoe", self.zoe.infer(image).shape) # (1, 1, 540, 960)
                    # print("Zoe2", self.zoe.infer(image).squeeze().shape) # (540, 960)
                    # print("Zoe3", depth_tensor.shape) # (540, 960, 1)

                    depth_tensor2 = self.MoGe.infer(image)["depth"].squeeze().unsqueeze(-1)
                    # print("Points", depth_tensor2["points"].shape) # (1, 540, 960, 3)
                    # print("Depth", depth_tensor2["depth"].shape) # (1, 540, 960)
                    # print("Mask", depth_tensor2["mask"].shape) # (1, 540, 960)
                
                # print("DEPTH TENSOR AKSDFMALSDKFMALSKDFMADLSKMFLAKSDMLFKMADSKLFMAKLSMF", depth_tensor2)
                depth_tensors.append(depth_tensor2)
